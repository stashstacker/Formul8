...
const singleFormulaParameterSchema = {
    ...
    properties: {
        ...
        unit: { type: Type.STRING, description: "The unit of measurement (e.g., 'm/s', 'kg', 'degrees'). Return null or empty string if unitless." },
...
const singleFormulaResponseSchema = {
    ...
    properties: {
        ...
        explanation: { type: Type.STRING, description: "A detailed explanation of the formula's purpose, principles, and how it works. Use Markdown formatting (bold, lists, etc) for readability." },
        parameters: {
            type: Type.ARRAY,
...
export const reverseEngineerFormula = async (csvData: string, context?: string): Promise<FormulaResult> => {
    const systemInstruction = "You are 'Data Detective', an expert AI data scientist and mathematician. Your goal is to analyze a provided dataset (inputs and outputs), deduce the underlying mathematical relationship or formula that best fits the data, and return a complete formula specification. You should identify if the data corresponds to a known scientific law or a custom linear/polynomial/exponential relationship. Ensure the generated code accurately implements the deduced formula. The code snippet should take the input columns as arguments. Try to infer parameter units from the context or data magnitude.";
    
    let contents = `Analyze the following dataset to find the underlying formula.\n\nDATASET (CSV):\n\`\`\`\n${csvData}\n\`\`\``;
    
    if (context && context.trim()) {
        contents += `\n\nContext/Hint about the data: "${context}"`;
    }

    const result = await callGemini<FormulaResult>(contents, systemInstruction, 0.4, singleFormulaResponseSchema);
